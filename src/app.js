...
class App extends React.Component {
  ...
  resetData = (init = false) => {
    let { showDialog, isShowGanttChartSetting } = this.state;
    ...
    if (init) {
      isShowGanttChartSetting = !this.isValidViewSettings(
  ...
  onTimelineSettingToggle = () => {
    this.setState({
      isShowGanttChartSetting: !this.state.isShowGanttChartSetting,
    });
  };

  onHideTimelineSetting = () => {
    this.setState({ isShowGanttChartSetting: false });
  };
  ...
  onAddView = (viewName) => {
    ...
    let isShowGanttChartSetting = !this.isValidViewSettings(settings);
    plugin_settings.views = updatedViews;
    this.setState(
      {
        plugin_settings,
  ...
  onSelectView = (viewId) => {
    ...
    if (viewIdx > -1) {
      ...
      let isShowGanttChartSetting = !this.isValidViewSettings(settings);
      this.setState({ selectedViewIdx: viewIdx, isShowGanttChartSetting });
  ...
  onTimelineClick = () => {
    const { isShowGanttChartSetting } = this.state;
    if (isShowGanttChartSetting) {
  ...
  render() {
    ...
    const { gridStartDate, gridEndDate } = this.getInitDateRange(settings);
    let rows = [];
    let groups = [];
    if (isValidSettings) {
      if (isGroupView) {
    ...
    return (
      ...
      >
        ...
        <Ganttchart
          ...
          isShowGanttChartSetting={isShowGanttChartSetting}
          dtable={this.dtable}
